...
class Product(models.Model):
    ...
    def get_all_products_by_barcode(self):
        print('get_all_products_by_barcode')
        products = self.env['product.product'].search_read(
            [('barcode', '!=', None), ('type', '!=', 'service')],
            ['barcode', 'display_name', 'uom_id', 'default_code', 'tracking']
        )
        ...
        products_to_read = self.env['product.product'].browse(list(set(t[1] for t in to_read))).sudo().read(['display_name', 'uom_id', 'default_code', 'tracking'])
