...
# REWRITE NAME OF PACKAGE ADD LOGBOOK AND BOX #
            packages = self.move_line_ids.mapped('result_package_id')
            if packages:
                next_package_number = str(len(packages))
            else:
                next_package_number = str(1)
            package.write({"name": package.name + '-' + pick.sale_id.log_book + '-' + '/CAJA #' + next_package_number})
        return package
